...
import { FaBars, FaTimes, FaSearch, FaMapMarkerAlt, FaCartPlus,FaUser } from "react-icons/fa";
import '../index.css'
    ...
    <div className='w-[12%] px-5 bg-gray-200 rounded-3xl hidden lg:inline-flex items-center'>
      <FaMapMarkerAlt size={20}/>
       ...
       <div className='flex pl-1 lg:pr-5 md:pr-2'>
        <Link to='/' className='hidden lg:inline-flex md:inline-flex justify-center bg-black text-white px-3 rounded-3xl items-center mr-4 '><FaCartPlus size={16}/> <p className='pl-1 font-semibold text-sm'>Cart</p><p className=' ml-1 border-gray-100 font-semibold text-sm px-1'> 0</p> </Link>
...
<div className={toggle ? `md:hidden  shadow-md  flex flex-col absolute w-[65%] left-0 top-0 bg-white right-0 z-[2] h-full  pt-3 pb-6 space-y-6 transition-all duration-500 ease-in-out  `: `absolute left-[-100%]`}>
 <div className='absolute right-0 px-4 py-2' onClick={handler}> <FaTimes size={24} /></div>
 <Link to='/login'> <button className='relative top-8 left-7 text-md w-[80%] cursor-pointer bg-gray-100 rounded-3xl px-4 py-2 font-semibold '>Login</button></Link>
  <Link to='/'><button className='relative top-5 left-7 w-[80%]  text-md cursor-pointer bg-black text-white rounded-3xl px-4 py-2 font-semibold'>Signup</button></Link>
</div>
